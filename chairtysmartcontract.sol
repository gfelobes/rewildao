...
contract CharloDAO is ReentrancyGuard, AccessControl {
    ...
    event ContributionReceived(address indexed fromAddress, uint256 amount); // event is like a class that is called whenever a new proposal is to be called 
    ...
    modifier onlyStakeholder(string memory message) { //modifier is used to enforce a certain criteria. Will be used with other functions to defien accessibility to that function 
...
// external is used to call a function (in this case modifier) outside the function. The function visibility is specified as external for economy of gas
...
//The vote() function is an external function that allows voting on proposals when called with the proposal’s id and a second true/false argument 
// depending on whether the vote is in support or against the proposal.
